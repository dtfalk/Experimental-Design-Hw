...
We will now write code that simulates toy data. This is helpful for testing out your code with data that you know should follow specific patterns. Note that it will look different from the raw example data. The dataframe should have the same dimensionality as the participant_level_data, with the same column names.
  ...
  3. First, generate a toy dataset called `toy_set_a` that shows a **large difference** in true recall proportion between the verbal and visual strategies. Here are the constraints:  
    ...
    3. Simulated data should include random noise, from a normal distribution with a mean between 0 and 1 and SD 0.1  
...
```
  ...
  4. Next, simulate a separate set of toy data called `toy_set_b` of the same size as the toy_set_a, that shows **no meaningful difference** in performance between verbal and visual strategies.  
    1. Simulated data should include random noise, from a normal distribution with a mean between 0 and 1 and SD 0.1.  
...
# Part 4: Plotting the data
  ...
  7. For each of `toy_set_a`, `toy_set_b`, and `participant_level_data`, plot the following:  
...
```
  ...
  Write your answers below:  
    By eyeballing the data I would guess that there is not a difference in true recall performance for this set of participants. Looking at the scatterplot and bar graphs, the actual data graphs looks much more similar to toy set b (the no difference dataset) than it does to toy set a graphs (the difference dataset). For the scatterplot and pargraph this is quite clear. For the histogram, it is difficult to tell which one looks more similar. I would say that the actual data *might* look more like toy set a, but it is very unclear. So in the final analysis two of the graphs really support the idea that the actual data is most similar to toy data set b (no difference) and one graph is inconclusive but leans towards toy data set a (there is a difference). As a result, I would guess that there is no difference in true recall proportion for the actual participants.
