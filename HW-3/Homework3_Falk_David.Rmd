...
# Total_SSR = All SSR in interference + SSR unique to encoding + encoding unique to VVIQ +
...
Adjusted R^2 penalizes models that use more predictors because they could just be overfitting. In this case R^2 is small (0.04) and adjusted R^2 is negative (-0.04). This means that the predictors are not improving the model beyond what would be expected by change and that the model may be including more predictors than are justified given the amount of variance explained. 
...
# R^2
Rsq_builtin <- summary(fit)$r.squared

# Adjusted R^2
Adj_Rsq_builtin <- summary(fit)$adj.r.squared

# F statistic
F_stat_builtin <- summary(fit)$fstatistic[1]

# F statistic p val
pval_F_builtin <- pf(summary(fit)$fstatistic[1],
...
cat(sprintf("Built-in p-value: %.6f", pval_F_builtin))
```
