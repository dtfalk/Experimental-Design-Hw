...
# Print the data
names(truerecall_resid_bestfit)
str(truerecall_resid_bestfit)
knitr::kable(
...
```{r}
overall_min_resid <- min(min(truerecall_resid_bestfit), min(falserecall_resid_bestfit))
overall_max_resid <- max(max(truerecall_resid_bestfit), max(falserecall_resid_bestfit))
plot(
     vviq_scores,
     ...
     ylab = "Residual",
     xlim = c(0, max(vviq_scores)),
     ylim = c(min(vviq_scores), overall_max_resid),
...
Regardless of vviq score the true recall residuals seem to vary around a mean of 0. Furthermore, the magnitude of the residuals do not seem to increase much as you progress along the x-axis (vviq score) so I would say that the true recall residuals are homoskedastic around a constant mean of 0. Since the mean always appears to be 0, this would suggest a slope and intercept near 0.     

The false recall residuals have larger magnitudes to their variance, and it seems that vviq scores greater than ~70 lead to low residuals while vviq scores between 40 and 70 seem to be around 0 but perhaps with a slight positive slope in that range.It is hard to eyeball the slope of the fit since I cannot do the calculation in my head, but unless all of this variance cancels out then we would see a line with non-zero slop and non-zero intercept even if the mean remains 0. So I am less sure what exactly this will mean for this dataset, but I can imagine there could be effects on mean, slope and intercept stemming from increased variance.
...
```{r}
coefs_truerecallresid  <- lm()
coefs_falserecallresid <-
